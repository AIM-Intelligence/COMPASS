...
def main():
    ...
    parser.add_argument('--company', type=str, help='Process only specified company')
    ...
    if args.company:
        # Process only specified company
        if args.company in company_names:
            company_names = [args.company]
            print(f"Processing only {args.company}")
        else:
            print(f"Company {args.company} not found in allowed_edge queries directory")
            return
    else:
