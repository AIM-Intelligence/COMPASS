...
def response2dict(response):
    ...
    if isinstance(response, dict):
        ...
        try:
            ...
            print(f"[WARNING] Failed to parse response content from OpenAI API. `response.choices[0].message.content`: {response.choices[0].message.content}. Original error: {str(e)}")
            structured_response = None
