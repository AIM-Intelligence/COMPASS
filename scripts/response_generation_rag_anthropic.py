"""
uv run python scripts/response_generation_rag_anthropic.py --model claude-sonnet-4-20250514
"""
...
DEFAULT_N_PROC = 50  # Default number of parallel processes
...
def process_single_query(
    ...
    try:
        response_text = call_anthropic_api_with_retry(
            model=model,
            messages=messages,
...
def call_anthropic_api_with_retry(
    ...
    """Call Anthropic chat completion API with retry logic."""
    ...
    if not api_key:
        raise RuntimeError(
            "ANTHROPIC_API_KEY environment variable is not set. Please set it in .env file or as environment variable."
        )
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                tqdm.write(
                    f"⚠️ Anthropic API call failed (attempt {attempt}/{retries}): {str(e)}"
                )
                tqdm.write(f"Retrying in {retry_delay} seconds...")
                time.sleep(retry_delay)
                retry_delay *= 1.5  # Exponential backoff
            else:
                tqdm.write(f"❌ All {retries} attempts failed for Anthropic API call")

    if last_exception is None:
        raise RuntimeError("Anthropic API call failed without raising an exception")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run evaluation for a given model, company, and query type using Anthropic Chat Completions.",
    )
    parser.add_argument("--system_prompt_dir", default=SYSTEM_PROMPT_DIR, help="Directory containing system prompts")
    parser.add_argument("--model", required=True, help="Anthropic model identifier (e.g., claude-3-5-sonnet-20241022)")
    parser.add_argument(
        "--company",
        action="append",
